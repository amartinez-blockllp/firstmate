...
fm_test_run_spawn() {
  ...
  # XDG_STATE_HOME is pinned empty for the same reason: a secondmate home's
  # task-worktree pool root is derived from it before HOME
  # (bin/fm-primary-scope-lib.sh), so an inherited value would name the
  # developer's own state directory; FM_TEST_XDG_STATE_HOME opts in.
  local spawn_home=$home/user-home
  mkdir -p "$spawn_home"
  FM_ROOT_OVERRIDE='' FM_HOME="$home" HOME="$spawn_home" \
    CLAUDE_CONFIG_DIR="${FM_TEST_CLAUDE_CONFIG_DIR:-}" \
    XDG_STATE_HOME="${FM_TEST_XDG_STATE_HOME:-}" \
