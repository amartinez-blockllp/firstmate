...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse` (2.2.0 or newer, because a remote-seeded home pools its task worktrees under its own root through `treehouse get --root`; see "Secondmate routes" in [`configuration.md`](configuration.md#secondmate-routes-datasecondmatesmd)), and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, or `kimi`; macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
