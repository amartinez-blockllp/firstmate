...
- The required remote tool set is `git`, `jq`, `herdr`, compatible `tasks-axi`, `treehouse` at the `treehouse get --root` floor that "Secondmate routes" in [`configuration.md`](configuration.md#secondmate-routes-datasecondmatesmd) owns, because a remote-seeded home pools its task worktrees under a root private to that home, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, or `kimi`; macOS additionally requires `lsof` so the doctor and guard can prove which process owns the session socket.
