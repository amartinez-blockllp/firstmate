...
A seeded home's task worktrees pool under its own `state/.treehouse/` rather than under the primary's pool, because treehouse keys a pool by repository identity and would otherwise hand a secondmate slots that are worktrees of the primary's clone of the same project; `bin/fm-wake-lib.sh`'s `fm_treehouse_pool_root` owns that contract, `bin/fm-spawn.sh` records the root as `treehouse_root=` in each task's metadata, and `bin/fm-teardown.sh` returns the slot under that recorded root.
