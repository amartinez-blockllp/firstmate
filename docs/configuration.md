...
A secondmate home's own Treehouse pools live outside the home, under the per-user state base; "Secondmate routes" below owns that per-home pool-root contract.
...
A pooled task in a secondmate home additionally records `treehouse_root=`, the per-home Treehouse pool root its slot was allocated under; an absent key means treehouse's default pool, and "Secondmate routes" below owns that contract.
...
A seeded home's task worktrees pool under a root private to that home rather than under the primary's pool, because treehouse keys a pool by repository identity and would otherwise hand a secondmate slots that are worktrees of the primary's clone of the same project; `bin/fm-primary-scope-lib.sh`'s `fm_treehouse_pool_root` owns that contract, `bin/fm-spawn.sh` records the root as `treehouse_root=` in each task's metadata, and `bin/fm-teardown.sh` returns the slot under that recorded root.
That root is `${XDG_STATE_HOME:-$HOME/.local/state}/firstmate/treehouse-pools/<id>-<hash>`, keyed by the secondmate id and a hash of the home's resolved path, and it sits outside the home on purpose: a home is a firstmate checkout whose `CLAUDE.md` imports firstmate's own `AGENTS.md`, and Claude Code loads the `CLAUDE.md` of every ancestor of a worker's directory.
A spawn refuses a root that would still land under a firstmate checkout, and retiring the home removes its root.
A slot allocated under the earlier in-home `state/.treehouse/` root keeps returning there at teardown, because teardown replays the recorded root.
That per-home pool needs treehouse 2.2.0 or newer for `treehouse get --root`; a secondmate home's bootstrap reports an older build as missing, and its spawn refuses before creating an endpoint, while a primary home never probes for it.
...
In a secondmate home that same check also requires `treehouse get --root` support, because its task worktrees pool under a root private to that home (see "Secondmate routes" above).
