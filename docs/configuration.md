...
In a secondmate home, `state/.treehouse/` additionally holds that home's own Treehouse pools; "Secondmate routes" below owns that per-home pool-root contract.
...
In a secondmate home that same check also requires `treehouse get --root` support, because its task worktrees pool under the home's own root (see "Secondmate routes" above).
