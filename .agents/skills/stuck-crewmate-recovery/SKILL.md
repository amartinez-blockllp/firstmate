...
Use `treehouse status` for treehouse-backed tmux, herdr, zellij, or cmux tasks, adding `--root` with the task's recorded `treehouse_root=` when that metadata is present, and use the recorded `orca_worktree_id=` and `terminal=` for Orca tasks.
